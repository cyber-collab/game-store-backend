  ...
  var DataBasicPlugin = function(context) {
    ...
    self.initialize = function() {
      ...
      var body = '<div class="form-group row-fluid">' +
          ...
          '<input class="ext-databasic-_test.blade.php form-control" type="text" />' +
          '</div>';
    ...
    self.getInfo = function() {
      ...
      if (rng.isOnData()) {
        ...
        if ($data.length) {
          ...
          return {
            node: $data,
            test: $data.attr('data-_test.blade.php'),
          };
        }
      }
    ...
    self.setContent = function($node) {
      $node.html('<p contenteditable="false">' + self.icon + ' ' + lang.databasic.name + ': ' +
        $node.attr('data-_test.blade.php') + '</p>');
    };

    self.updateNode = function(info) {
      self.setContent(info.node
        .attr('data-_test.blade.php', info.test));
    };
    ...
    self.openDialog = function(info) {
      return $.Deferred(function(deferred) {
        var $inpTest = self.$dialog.find('.ext-databasic-_test.blade.php');
    ...
    lang: {
      'en-US': {
        databasic: {
          ...
          testLabel: '_test.blade.php input',
        },
      },
    },
